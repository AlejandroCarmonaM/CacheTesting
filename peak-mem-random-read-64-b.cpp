...
void init() {
  ...
  a = static_cast<long long int*>(aligned_alloc(64, options.array_size * sizeof(long long int)));

  // Fill the array 'a' with sequential values
  for (size_t i = 0; i < options.array_size; ++i) {
    a[i] = static_cast<long long int>(i);
  }

  // Shuffle the indices to randomize the order
  random_device rd;
  mt19937 g(rd());
  shuffle(a, a + options.array_size, g);
}
...
void calculate() {
  ...
  long long int *la = assume_aligned<64>(a);
  volatile long long int idx = 0;
  size_t cont=0;

  while(cont < options.array_size){
    idx = la[idx];
    cont++;
  }
}
...
void measure()
{
  long n_bytes = options.array_size * sizeof(long long int);
...
int main(int argc, char **argv)
{
  ...
  printf("Benchmark: Random reads 64 bits spec-b\n");
  executeTest("L1", (L1_CACHE_SIZE / sizeof(long long int)) * REDUCE_SIZE);
  executeTest("L2", (L2_CACHE_SIZE / sizeof(long long int)) * REDUCE_SIZE);
  executeTest("L3", (L3_CACHE_SIZE / sizeof(long long int)) * REDUCE_SIZE);
